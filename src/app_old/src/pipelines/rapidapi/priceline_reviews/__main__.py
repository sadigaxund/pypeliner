####################################################
# PACKAGE:  src.extract.rapidapi.priceline_reviews
# ENTRY:    __main__.py
# AUTHOR:   Sadig Akhund
...
#
# DESCRIPTION:
# This script extracts reviews of Priceline.com listings from Rapidapi data provider.
#
####################################################


from src.pipelines.rapidapi.priceline_reviews import *
...
def extract_reviews_per_listing(
    ...
    extract_task = Task(
        ...
        Metadata={
            ...
            "page_size": page_size,
            "offset": offset,
            "start_date": Configs.EXTRACT_START_DATE.isoformat(),
            "end_date": Configs.EXTRACT_END_DATE.isoformat(),
        }
    )

    @handle(
        error_message=MSG_ERROR_SOURCE_EXTRACT,
        strict=False,
        do_raise=False,
        timeout_secs=5,
        max_retries=3,
    ...
    def _extract_reviews() -> List[Dict]:
        ...
        reviews = response.json()\
            ...
            .get('results', {})\
            .get('review_data', {}) or {}
        return reviews.values(), extract_task
    ...
    for index, review in enumerate(results):
        review_date_str = review['creation_date'] + " 00:00:00"
        ...
        # TRANSFORM REVIEWS
        review['hotel_id_ppn'] = listing['hotel_id_ppn']
        review['hotel_id_t'] = listing['hotel_id_t']
