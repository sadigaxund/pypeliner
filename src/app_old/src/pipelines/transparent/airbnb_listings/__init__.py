...
from src.templates.Statistics import TRANSPARENT_LISTINGS_STATISTICS as STATISTICS
from urllib.parse import quote_plus

Config(CODE_NAME='airbnb_listings')
...
def statistics_report() -> None:
    ...
    STATISTICS['Extra']['Completion'] = safe_divide(
        ...
        STATISTICS['Success']['Extract'] - STATISTICS['Extra']['Discarded']
    )
    printlog("STATISTICS", "trace", STATISTICS)


if __name__ == "src.extract.transparent.airbnb_listings":
    atexit.register(destructor)
    constructor(script_title="Airbnb Listings Data Extractor",
                cache_table='airbnb',
                code_name=Configs.CODE_NAME)
    define_schedule(
        schedule=Configs.SCHEDULE,
        DSD=last_month(set_day=31),
        DED=today(midnight=False),
